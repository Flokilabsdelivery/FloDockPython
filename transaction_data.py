...
#    print(row['spell_check'])
    unique = deduplicate_strings(row['spell_check'],80)

    row['unique'] = unique

#    duplicates = list(set(row['spell_check']) - set(unique))
...
#                print(len(file1[config['CustomerDOB']].unique()))
...
#                print(df[config['CustomerAddress']])
...
#                df['CONTACT_DETAILS'].str.startswith('9'$ # df.loc[df_valid,'valid'] = 'valid'
...
#                final_df.to_csv('frank.csv',index = False)
...
#                print(len(duplicate_hash_df))

#                print(duplicate_hash_df['FIRSTNAME'])
...
#                print('after hashcode check')
 ...
 #               print(len(final_df))
                ...
                final_df['path'] = i.replace(config['replace_string'],config['replace_with'])+"//valid//"+transaction_file.replace('.csv','')+"_CDMS_output.csv"
...
#                invalid_records = pd.concat([invalid_records,duplicates],axis = 0)
                ...
                print('valid records',len(final_df))


                print('corporate',len(corporate_customers))
...
#                    my_dict = {'fileUploadId': upload_id, 'filePath': i.replace(config['replace_string'],config['replace_with'])+"//valid//", 'fileName': transaction_file.replace('.csv','')+"_CDMS_output.csv","subListID":85,"businessHierarchyId":23}
...
#                    producer.close()
                ...
                with open('response.txt','w') as w:
                    ...
                    w.write(text+str(count)+"."+i+transaction_file+"\n")
...
#        else:
            ...
            with open('response.txt','w') as w:
                ...
                w.write(text+"Error at file:"+str(count)+"."+i+transaction_file+"\n")
